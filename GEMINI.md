...
- For existing stylings, keep that as it is.
- For new stylings, use the mantine stylings.
...
- as you can se ei havw added server.ts inside routeFileIgnorePatters in vite config, if the route has any server function which is used only in that route, create a folder for that route and add the route code inside routefolder/route.tsx and add the server function inside routefolder/server.ts.
